...
range_str = '(\S+)--(\S+)'
range_r = re.compile(range_str)
born_r = re.compile('\((\d\d\d\d)--')
died_r = re.compile('--(\d\d\d\d)\)')
year_r = re.compile('\d\d\d\d')
voice_r = re.compile('\d+')
partiture_r = re.compile('yes')


def get_year(line):

    match = re.search(year_r, line)
...
def get_voice(line, voice_num):
    ...
    match = re.search(range_r, line)
    voice_r, name = None, None

    if match:
        voice_r = match.group(0)
        name = re.sub(range_str, "", line)
    else:
        name = line

    if (name.strip() == '') and (not voice_r):
        return None

    return Voice(name.strip(), voice_r, voice_num)
...
def get_composers(composers, conn):
    ...
    for composer in composers:
        ...
        match = re.search(born_r, composer)
        if match:
            born = match.group(1)
...
def process_print(lines, conn):
    ...
    for line in lines:
        ...
        if key_val[0] == 'Print Number':
            ...
            voice_num = 1
            match = re.search(voice_r, key_val[0])
            if match:
